...
export class ValidatorsHttp {
    async queryValidatorsFromLcd(validatorStatus: boolean, pageNum: number, pageSize: number): Promise<VValidatorDto[]>{
        let validatorsLcdUrl: string;
        if (validatorStatus === null) {
            validatorsLcdUrl = `${cfg.serverCfg.lcdAddr}/node/validators?page=${pageNum}&limit=${pageSize}`
        } else {
            validatorsLcdUrl = `${cfg.serverCfg.lcdAddr}/node/validators?jailed=${validatorStatus}&page=${pageNum}&limit=${pageSize}`
        }
        try {
