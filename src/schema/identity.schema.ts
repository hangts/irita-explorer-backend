...
import {
    IIdentityInfoQuery,
    IIdentityInfoResponse,
    IIdentityByAddressQuery
...
import {DuplicateKey, hubDefaultEmptyValue} from "../constant";
...
IdentitySchema.statics = {
    ...
    async insertIdentityInfo(IdentityInfo) {
        await this.insertMany(IdentityInfo, {ordered: false}, (error: any) => {
    ...
    async updateIdentityInfo(updateIdentityData) {
        ...
        if (updateIdentityData.credentials && updateIdentityData.credentials !== hubDefaultEmptyValue) {
            ...
            await this.updateOne({identities_id}, {
                credentials,
                update_block_time,
                update_block_height,
                update_tx_hash,
