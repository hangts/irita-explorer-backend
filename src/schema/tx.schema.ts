...
TxSchema.statics.queryCallServiceWithConsumerAddr = async function(consumerAddr: string, txId: number, limit: number): Promise<ListStruct> {
    const result: ListStruct = {};
...
TxSchema.statics.queryBindServiceWithProviderAddr = async function(ProviderAddr: string, txId: number, limit: number): Promise<ListStruct> {
    const result: ListStruct = {};
    const queryParameters: any = {
...
TxSchema.statics.queryLatestHeight = async function(height :number, status?: string): Promise<ITxStruct> {
    ...
    return await this.findOne(query,{height:1}).sort({ time: -1 });
};
...
TxSchema.statics.queryServiceRespondTx = async function(serviceName: string, provider: string, txId: number, limit: number): Promise<ITxStruct[]> {
    const queryParameters: any = {
        'msgs.type': TxType.respond_service,
    };
    if (serviceName && serviceName.length) {
...
TxSchema.statics.queryListByCreateAndUpDateIdentity = async function(
  ...
  limitSize:number,
    ...
    return await this.find(params, dbRes.syncIdentityTask ).limit(limitSize).sort({time:1})
}
...
TxSchema.statics.queryDenomTxList = async function (lastBlockHeight: number): Promise<ITxStruct[]>  {
    return await this.find(
        ...
        }, { msgs: 1, height: 1, time: 1, tx_hash: 1 }).sort({ time: 1 }).limit(MAX_DENOM_TX_COUNT);
};
//used at the nft_cron_task
...
TxSchema.statics.queryVoteByProposalIdAll = async function (id:number): Promise<ITxVoteProposalAll[]>  {
    ...
    return await this.find(params,dbRes.voteList).sort({time:1});
};
...
TxSchema.statics.queryVoteByTxhashs = async function (hash: string[], query?: PagingReqDto): Promise<IListStruct>  {
    ...
    if (query) {
        ...
        data = await this.find(queryParameters,dbRes.voteList).sort({ time: -1 })
    }
    return data;
};
...
TxSchema.statics.queryVoteByAddr = async function (address:string): Promise<ITxVoteALL[]>  {
    ...
    return await this.find(queryParameters,dbRes.voteList).sort({ time: 1 });
};
...
TxSchema.statics.queryAccountTxList = async function (lastSyncBlockHeight:number): Promise<ITxStruct[]>  {
    ...
    },{addrs: 1,height: 1,_id:0}).sort({time:1});
};

TxSchema.statics.queryTxMaxHeight = async function (): Promise<ITxStruct[]>  {
    return await this.find({},{height: 1}).sort({time: -1}).limit(1);
};
